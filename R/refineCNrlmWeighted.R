...
#  Weight <- mean(Tinput) * dim(Tinput)[2]/3;
#  outputRlm <- rlm(rbind(t(Tinput[,refs]),c(Weight,-Weight)), rbind(matrix(data=2, nrow=ncol(Tinput[,refs]), ncol=1),0),maxit=50);
  outputRlm <- rlm(t(Tinput[,refs]), matrix(data=2, nrow=ncol(Tinput[,refs]), ncol=1),maxit=50);
...
#  matDiff <- rlm(rbind(t(Tinput[,refs]),c(Weight,Weight)), c(naiveGenoDiff,0),maxit=50, weights=coeffs);
